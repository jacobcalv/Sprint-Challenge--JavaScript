...
const universities = [];
graduates.forEach(graduate => (universities.push(graduate['first_name'])));
...
const uni = [];
graduates.filter(graduate => {if (graduate['university'].includes('Uni')) {uni.push(graduate['university'])}})
console.log(uni);
...
zooAnimals.filter(animal => {if (animal['population'] < 5  ) {lowerPopulation.push(animal['animal_name'])}});
...
const populationTotal = zooAnimals.reduce((population, animal) => {
  return population = population + animal.population;
},0);
